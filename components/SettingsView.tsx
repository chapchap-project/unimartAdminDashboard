...
const SettingsView: React.FC = () => {
  ...
  const [useMock, setUseMock] = useState(api.isMockMode());
  ...
  const handleTestConnection = async () => {
    ...
    const originalMock = api.isMockMode();
    
    api.setBaseUrl(baseUrl);
    api.setUseMock(false); // Force live for test
            ...
            <div className="space-y-6">
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input 
                            ...
                            disabled={useMock}
                            className={`w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-all ${
                                useMock ? 'bg-slate-50 text-slate-400 border-slate-200' : 'bg-white text-slate-800 border-slate-300'
                            }`}
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <span className="block text-sm font-bold text-slate-700">Mock Data Mode</span>
                            <span className="text-xs text-slate-500">Use local data for testing UI without backend</span>
                        </div>
                    </div>
                    <button 
                        onClick={() => setUseMock(!useMock)}
                        className={`transition-colors ${useMock ? 'text-indigo-600' : 'text-slate-400'}`}
                    >
        ...
        <div className="bg-indigo-900 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden">
            ...
            <div className="mt-8 pt-6 border-t border-indigo-700/50">
                ...
                <div className="flex items-center gap-2 font-mono text-sm">
                    <div className={`w-2 h-2 rounded-full ${useMock ? 'bg-amber-400' : connectionStatus === 'SUCCESS' ? 'bg-emerald-400' : 'bg-red-400'}`}></div>
                    {useMock ? 'Local Mock Server' : baseUrl}
                </div>
            </div>
        </div>
      </div>
